...
''');
  ...
  test('shell init persists branches and generates StatefulShellRoute', () async {
    ...
    expect(
      config.routing.shell.branches.map((item) => item.path),
      <String>['/', '/blog', '/profile'],
    );
    ...
    expect(await generated.readAsString(), contains('StatefulShellRoute.indexedStack'));
    expect(
      File('${project.root.path}/lib/app/router/generated/branches/blog_branch.dart')
          .existsSync(),
      isTrue,
    );
    expect(
  ...
  test('generates root route centrally without feature-owned routes', () async {
    ...
    expect(
      File('${project.root.path}/lib/app/router/generated/features/about_route.dart')
          .existsSync(),
      isTrue,
    );
    expect(
      File('${project.root.path}/lib/features/about/presentation/routes')
          .existsSync(),
      isFalse,
    );
  ...
  test('unknown branch fails before generating the feature', () async {
    ...
    await expectLater(
      ...
      throwsA(
        predicate(
          (error) =>
              error.toString().contains('Shell branch "missing" does not exist') &&
    ...
    expect(Directory('${project.root.path}/lib/features/details').existsSync(), isFalse);
  });
  ...
  test('MVC uses the same central branch routing flow', () async {
    ...
    expect(routeFile, contains('features/account/views/screens/account_screen.dart'));
    expect(
      Directory('${project.root.path}/lib/features/account/routes').existsSync(),
      isFalse,
    );
  });
  ...
  test('dry run does not modify routing files or kite.yaml', () async {
    ...
    expect(Directory('${project.root.path}/lib/features/preview').existsSync(), isFalse);
  });
  ...
  test('route registration is idempotent', () async {
    ...
    expect(
      RegExp('blogDetails').allMatches(appRoutes).length,
      1,
    );
    final branch = await File(
      '${project.root.path}/lib/app/router/generated/branches/blog_branch.dart',
    ...
    expect(
      Directory('${project.root.path}/lib/features/details/presentation/routes')
          .existsSync(),
      isFalse,
    );
  });
  ...
  test('installed template dependencies are not regenerated by features', () async {
    final project = await createProject(shell: false);
    final observer = File(
